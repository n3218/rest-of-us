...
const HomeGuest = () => {
  ...
  const homeGuestReducer = (draft, action) => {
    switch (action.type) {
      case "usernameImmediately":
        ...
        if (draft.username.value.length > 30) {
          ...
          draft.username.message = "Username cannot exeed 30 characters."
        }
      ...
      case "passwordImmediately":
        ...
        if (draft.password.value.length > 50) {
          ...
          draft.password.message = "Password cannot exeed 50 characters."
        }
        return
  ...
  const [state, dispatch] = useImmerReducer(homeGuestReducer, initialState)
  ...
  useEffect(() => {
    if (state.username.checkCount) {
      ...
      const fetchResults = async () => {
        try {
          const response = await Axios.post(`/doesUsernameExist`, { username: state.username.value }, { cancelToken: searchRequest.token })
  ...
  useEffect(() => {
    if (state.email.checkCount) {
      ...
      const fetchResults = async () => {
        try {
          const response = await Axios.post(`/doesEmailExist`, { email: state.email.value }, { cancelToken: searchRequest.token })
        ...
        <div className="col-lg-5 pl-lg-5 pb-3 py-lg-5">
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              <input onChange={e => dispatch({ type: "emailImmediately", value: e.target.value })} id="email-register" name="email" className="form-control" type="text" placeholder="email@example.com" autoComplete="off" />
